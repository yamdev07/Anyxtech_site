...
export default async function EditCollectionPage({ params }: { params: Promise<{ collection: string }> }) {
  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 space-y-8">
      <div className="dash-hero">
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/dashboard/edit/${collection}/create`} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-indigo-600 shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl">
      ...
      <PaginatedList collection={collection} path={`/dashboard/edit/${collection}`} gradient={meta.gradient} icon={meta.icon} />
    </div>
  );
}
