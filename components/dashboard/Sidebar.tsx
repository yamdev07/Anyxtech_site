...
export default function Sidebar({ email }: { email?: string | null }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-[var(--border)] bg-[var(--card)]/80 backdrop-blur-2xl py-5 transition-transform lg:translate-x-0 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between px-5 pb-5 border-b border-[var(--border)]">
          <Link href="/dashboard" className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-brand-blue to-brand-light shadow-lg shadow-brand-light/20">
                ...
                {!isCollapsed && group.items.map((n) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`group relative flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-[13px] font-medium transition-all mb-0.5 ${
                        active
                          ? "bg-gradient-to-r from-brand-blue/30 to-brand-light/10 text-brand-light border border-brand-light/20"
                          : "text-[var(--text-soft)] hover:bg-brand-light/5 hover:text-[var(--text)]"
                      }`}
                    >
                      {active && (
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-[60%] w-[3px] rounded-r-full bg-brand-light" />
                      )}
        ...
        <div className="border-t border-[var(--border)] px-4 pt-4 pb-3 space-y-2">
          <div className="flex items-center gap-3 rounded-xl px-3 py-2.5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-brand-blue to-brand-light text-sm font-bold text-white shadow-lg shadow-brand-light/20">
              {userInitial}
            </div>
          ...
          <div className="flex gap-1">
            <Link
              href="/"
              target="_blank"
              className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-xs font-medium text-[var(--text-soft)] transition-colors hover:border-brand-light/30 hover:text-brand-light"
            >
              <ExternalLink className="h-3.5 w-3.5" /> Site
            </Link>
            <a
              href="/admin/logout"
              className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-xs font-medium text-[var(--text-soft)] transition-colors hover:border-red-500/30 hover:text-red-400"
            >
